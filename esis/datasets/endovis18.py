...
class EndoVis18Adapter(BaseDatasetAdapter):
    dataset_name = "endovis18"
    index_file_name = "endovis18_index.json"

    def collect_samples(self) -> list[DatasetSample]:
        root = self.dataset_root / "data" / "endovis2018"
        if not root.exists():
            ...
            for image_path in sorted(path for path in image_dir.iterdir() if path.is_file()):
                ...
                samples.append(
                    DatasetSample(
                        ...
                        metadata={
                            "split_group": "train" if split_dir.name == "train" else "validation",
                        },
                    )
                )
        return samples
